...
var consoleReporter = {
    ...
    specDone: (spec) => {
        ...
        if(!spec.failedExpectations.length){
            ...
            wdog.increment(consoleReporter.statsDServiceName.trim() + spec.fullName.trim(), ['status:passed']);
        } else {
            log(colors.fail() + spec.fullName.trim());
            wdog.increment(consoleReporter.statsDServiceName.trim() + spec.fullName.trim(), ['status:failed']);
